  ...
  <div class="currency-pair-list">
    <el-card class="filter-card">
      <el-form :inline="true" :model="queryForm">
        <el-form-item label="关键字">
          <el-input v-model="queryForm.keyword" placeholder="货币对编码/货币" clearable @keyup.enter="handleQuery" />
        </el-form-item>
    ...
    <el-drawer v-model="drawerVisible" :title="drawerTitle" direction="rtl" size="480px">
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="卖方小数位" prop="askDecimal">
          <el-input-number v-model="formData.askDecimal" :min="0" :max="10" />
        </el-form-item>
...
import { listCurrencyPair, saveCurrencyPair, updateCurrencyPair, deleteCurrencyPair } from '@/api/basedata'
...
const drawerTitle = computed(() => (formData.id ? '编辑币种对' : '新增币种对'))
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await listCurrencyPair(params)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该币种对吗?', '提示', { type: 'warning' })
    await deleteCurrencyPair(row.id)
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        if (formData.id) {
          await updateCurrencyPair(formData)
          ElMessage.success('更新成功')
        } else {
          await saveCurrencyPair(formData)
