...
**UI测试必须包含功能验证：**
- 不仅仅是检查元素是否存在
- 必须验证操作后的实际功能效果（如：点击保存后数据是否真正保存）
- 必须验证数据展示的正确性（如：金额显示格式、小数位精度）
- 必须验证用户交互流程的完整性
...
- 任何操作（增删改查）后必须检查错误提示
- 即使操作成功，也要确认无隐藏错误
- 错误日志必须包含完整响应内容（包含HTTP状态码、响应body、请求详情）

**慢请求记录：**
- 响应时间超过1秒的请求需要记录到测试报告
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.2 | 2026-06-14 | 添加测试质量强制要求：UI功能验证、API数据验证、错误检查、完整错误日志 |
