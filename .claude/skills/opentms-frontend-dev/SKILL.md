...
**目的**：通过编译构建验证代码准确性，确保无语法错误、导入错误、类型错误。
...
2. **检查构建结果**
   - 构建成功（无 error 输出）→ 验证通过，进入步骤8
   - 构建失败（有 error 输出）→ 记录错误信息，进入步骤8进行问题分析

3. **常见错误类型及处理**
   | 错误类型 | 典型表现 | 可能原因 |
   |----------|----------|----------|
   | 语法错误 | `SyntaxError` | 模板语法不符合Vue3规范 |
   | 导入错误 | `Cannot find module` | 路径错误或大小写不匹配 |
   | 组件未注册 | `component not registered` | 忘记在template中引入或命名错误 |
   | API路径错误 | 404 on API call | URL路径与后端不一致 |
   | 响应结构不匹配 | `res.data undefined` | 后端返回结构与前端期望不一致 |
...
**目的**：当编译出错时，找到出问题的点，分析根本原因，并优化本skill避免下次遇到相同问题。
...
1. **错误定位**：错误发生在哪个文件、哪一行
2. **错误分类**：
   - **Skill指导问题**：skill提供的模板/规范有误导致
   - **开发人员问题**：实现时偏离了skill指导
   - **环境问题**：依赖/配置问题
3. **根因追溯**：
   - 如果是Skill模板问题 → 定位到skill中具体的模板/规范
   - 如果是实现偏差 → 定位到skill中哪一步指引不清晰
   - 如果是新问题 → 确认是否为skill未覆盖的场景
...
| 后续Skill | 触发条件 | 输出 |
|-----------|----------|------|
| 测试用例设计 | 页面开发完成且编译验证通过 | 测试用例 |
| 代码审查 | 编译验证通过后 | Review意见 |
| Skill优化 | 编译验证失败时 | 更新skill内容 |
...
| 检查项 | 标准 | 权重 |
|--------|------|------|
| 规范符合性 | 符合项目命名和结构规范 | 15% |
| 功能完整性 | 实现所有原型需求 | 20% |
| 交互正确性 | 符合UX交互规范 | 15% |
| 代码可维护性 | 结构清晰、可复用 | 10% |
| 错误处理 | 完善的异常处理 | 10% |
| 性能表现 | 无明显性能问题 | 10% |
| **编译验证** | **构建成功，无语法/导入错误** | **20%** |
...
| v1.1 | 2026-05-27 | 新增编译验证步骤(步骤7)和问题分析与Skill优化步骤(步骤8)，要求开发完成后必须通过`npm run build`验证，发现错误时分析根因并优化skill |
